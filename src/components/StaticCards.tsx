...
interface QuadernoEditorProps {
  playSoundBlip?: (freq: number, type?: OscillatorType, duration?: number) => void;
}

const BOOKS = [
  { title: "Grammatica Cinese", icon: "📒", gradient: "from-blue-100 to-blue-200 border-blue-300" },
  { title: "Idee", icon: "💡", gradient: "from-green-100 to-green-200 border-green-300" },
  { title: "Vari", icon: "🧾", gradient: "from-purple-100 to-purple-200 border-purple-300" },
  { title: "Coreano", icon: "🏯", gradient: "from-orange-100 to-orange-200 border-orange-300" },
  { title: "Storie Cinesi", icon: "🏮", gradient: "from-red-100 to-red-200 border-red-300" },
  { title: "Storie Coreane", icon: "🥢", gradient: "from-teal-100 to-teal-200 border-teal-300" },
  { title: "Codici", icon: "💻", gradient: "from-pink-100 to-pink-200 border-pink-300" },
  { title: "Internet", icon: "🌍", gradient: "from-yellow-100 to-yellow-200 border-yellow-300" },
];

export const QuadernoEditorCard: React.FC<QuadernoEditorProps> = ({ playSoundBlip }) => {
  // Carica i dati delle note memorizzati localmente nel localStorage
  const [bookNotes, setBookNotes] = useState<{ [key: string]: string }>(() => {
    const saved = localStorage.getItem("bookNotes");
    return saved ? JSON.parse(saved) : {};
  });

  const [currentBook, setCurrentBook] = useState<string>("general");
  const [notesText, setNotesText] = useState<string>("");
  const [saveStatus, setSaveStatus] = useState<string>("Quadretti extra-grandi");
  
  const saveTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Aggiorna l'area note quando l'utente seleziona un quaderno diverso
  useEffect(() => {
    setNotesText(bookNotes[currentBook] || "");
  }, [currentBook]);

  // Pulisce i timer all'uscita dal componente
  useEffect(() => {
    return () => {
  ...
  const handleNotesChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Riproduce un delizioso suono tattile vintage randomico mentre digiti!
    if (playSoundBlip && Math.random() > 0.82) {
      playSoundBlip(900, "triangle", 0.015);
    }

    // Salvataggio automatico ultra-fluido (Debounced a 1000ms)
    if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);

    saveTimeoutRef.current = setTimeout(() => {
      setBookNotes((prev) => {
        const updated = { ...prev, [currentBook]: newText };
        localStorage.setItem("bookNotes", JSON.stringify(updated));
        return updated;
      });
      setSaveStatus("✅ Salvato!");
      setTimeout(() => {
        setSaveStatus("Quadretti extra-grandi");
      }, 1500);
    }, 1000);
  };

  const handleBookSelect = (title: string) => {
    if (playSoundBlip) playSoundBlip(800, "sine", 0.05);
    setCurrentBook(title);
  };

  return (
    <div className="desk-card bg-gradient-to-br from-blue-50 to-indigo-100 p-4 rounded-3xl shadow-xl w-full flex flex-col select-text" style={{ minHeight: "520px" }}>
      {/* Header Intestazione */}
      <header className="bg-white shadow-md rounded-2xl p-3 mb-4 shrink-0 border border-indigo-50">
        <div className="text-center">
          <h1 className="text-xl md:text-2xl font-bold text-blue-900 flex items-center justify-center gap-2">
            <span>📝</span> <span>Quaderno Editor</span>
          </h1>
        </div>
      </header>

      {/* Main Grid: Navigazione Libri + Foglio Note */}
      <div className="flex flex-col md:flex-row gap-4 flex-1 min-h-[380px]">
        {/* Sinistra: Elenco dei Quaderni */}
        <div className="w-full md:w-[240px] h-[160px] md:h-auto shrink-0 bg-white rounded-2xl shadow-inner flex flex-col overflow-hidden border border-gray-100">
          <div className="p-2.5 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 sticky top-0 z-10">
            <h2 className="font-bold text-xs md:text-sm text-gray-800 flex items-center gap-1.5">
              <span>📖</span> <span>I Miei Quaderni</span>
            </h2>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
            {/* Pulsante Note Generali di Default */}
            <button
              onClick={() => handleBookSelect("general")}
              className={`w-full transition-all text-left duration-200 hover:-translate-y-px rounded-lg p-2.5 border-2 flex items-center gap-2.5 cursor-pointer ${
                currentBook === "general"
                  ? "bg-blue-50 border-blue-605 shadow-sm scale-[1.01]"
                  : "bg-gray-50/50 border-gray-250/60"
              }`}
            >
              <span className="text-xl">📓</span>
              <h3 className="font-extrabold text-gray-800 text-xs text-ellipsis overflow-hidden">Note Generali</h3>
            </button>

            {/* Ciclo dei restanti Quaderni */}
            {BOOKS.map((book) => (
              <button
                key={book.title}
                onClick={() => handleBookSelect(book.title)}
                className={`w-full transition-all text-left duration-200 hover:-translate-y-px rounded-lg p-2.5 border-2 flex items-center gap-2.5 cursor-pointer bg-gradient-to-r ${
                  book.gradient
                } ${
                  currentBook === book.title
                    ? "border-blue-600 shadow-sm scale-[1.01]"
                    : "border-transparent"
                }`}
              >
                <span className="text-xl">{book.icon}</span>
                <h3 className="font-extrabold text-gray-800 text-xs leading-tight">{book.title}</h3>
              </button>
            ))}
          </div>
        </div>

        {/* Destra: Foglio di carta a quadretti */}
        <div className="flex-1 bg-white rounded-2xl shadow-md overflow-hidden border border-gray-200 flex flex-col">
          <div className="p-3 border-b border-gray-250 bg-gradient-to-r from-green-50 to-emerald-50 shrink-0 flex justify-between items-center">
            <div>
              <h2 className="font-black text-sm md:text-base text-gray-800 uppercase tracking-wide">
                {currentBook === "general" ? "Note Generali" : `Note: ${currentBook}`}
              </h2>
              <p className={`text-[10px] uppercase font-bold tracking-wider transition-colors duration-300 ${saveStatus.includes("✅") ? "text-emerald-600 font-black" : "text-gray-400"}`}>
                {saveStatus}
              </p>
            </div>
          </div>
          
          <div className="flex-1 flex flex-col p-3 bg-stone-50">
            <textarea
              value={notesText}
              onChange={handleNotesChange}
              className="w-full h-full flex-grow p-4 resize-none focus:outline-none text-base md:text-lg font-mono text-gray-950 border border-gray-200 rounded-xl shadow-inner bg-white select-text"
              style={{
              ...
              placeholder="Inizia ad annotare pensieri, appunti o caratteri in questo spazio..."
            />
          </div>
        </div>
      </div>
